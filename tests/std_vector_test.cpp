...
int main()
{
    // INITIALIZATION
    std::vector<int> one;
    std::vector<int> two(10, 90);
    std::vector<int> three(two.begin(), two.end());
    std::vector<int> tour(two);

    for (std::vector<int>::iterator x = two.begin(); x != two.end(); ++x)
    ...
    std::cout << "\nMy vector " << '\n';
    for (std::vector<int>::iterator one = first.begin(); one != first.end(); ++one)
        std::cout << ' ' << *one;
    ...
    // INSERT
    std::vector<int> insert_test(3, 100);
    ...
    // SWAP
    std::vector<int> a(3, 100); // three ints with a value of 100
    std::vector<int> b(5, 200); // five ints with a value of 200
