...
int main()
{
    ...
    cap_test.insert(std::pair<char, int>('d', 40));
    ...
    // ACCESS OPERATOR
    std::map<char, std::string> op_test;
    ...
    // INSERT/ERASE
    std::map<char, int> ins_test;

    // first insert function version (single parameter):
    ins_test.insert(std::pair<char, int>('a', 100));
    ins_test.insert(std::pair<char, int>('z', 200));

    std::pair<std::map<char, int>::iterator, bool> ins_pair;
    ins_pair = ins_test.insert(std::pair<char, int>('z', 500));
    ...
    std::map<char, int>::iterator ins_it = ins_test.begin();
    ins_test.insert(ins_it, std::pair<char, int>('b', 300)); // max efficiency inserting
    ins_test.insert(ins_it, std::pair<char, int>('c', 400)); // no max efficiency inserting
    ...
    for (std::map<char, int>::iterator it = mod_test.begin(); it != mod_test.end(); ++it)
        std::cout << it->first << " => " << it->second << '\n';
    ...
    // KEY COMPARISON
    std::map<char, int> key_test;

    std::map<char, int>::key_compare mycomp = key_test.key_comp();
    ...
    std::map<char, int>::iterator key_it = key_test.begin();
    do
    {
    ...
    std::pair<char, int> highest = *comp_test.rbegin(); // last element

    std::map<char, int>::iterator comp_it = comp_test.begin();
    do
    {
    ...
    // FIND / COUNT
    std::map<char, int> find_test;
    std::map<char, int>::iterator find_it;
    ...
    // print content:
    for (std::map<char, int>::iterator it = low_up.begin(); it != low_up.end(); ++it)
        std::cout << it->first << " => " << it->second << '\n';

    // EQUAL
    std::map<char, int> equal_test;
